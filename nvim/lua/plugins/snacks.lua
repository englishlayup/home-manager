return {
  ...
  keys = {
    { '<leader>sf', function() Snacks.picker.smart() end,                   desc = 'Smart [S]earch [F]iles' },
    {
      '<leader>b',
      function()
        Snacks.picker.buffers { filter = { filter = function(item)
          return item
              .buf and vim.bo[item.buf].buftype ~= 'terminal'
        end } }
      end,
    ...
    },
    {
      ...
      desc = '[S]earch [T]erminal Buffers'
    },
    ...
    { '<leader>n',  function() Snacks.picker.grep { cwd = '~/notes' } end,  desc = '[G]rep [N]otes' },
